...
function SubdataSelectionTester(url, auth, auth_cb, timeout, async_job_check_time_ms, service_version) {
    var self = this;
    ...
    var _auth_cb = auth_cb;
     ...
     this.get_object_subset = function (sub_object_ids, _callback, _errorCallback) {
        ...
        return json_call_ajax(_url, "SubdataSelectionTester.get_object_subset",
            [sub_object_ids], 1, _callback, _errorCallback);
    };
 
     this.hello_world = function (name, _callback, _errorCallback) {
        if (typeof name === 'function')
            throw 'Argument name can not be a function';
        if (_callback && typeof _callback !== 'function')
            throw 'Argument _callback must be a function if defined';
        if (_errorCallback && typeof _errorCallback !== 'function')
            throw 'Argument _errorCallback must be a function if defined';
        if (typeof arguments === 'function' && arguments.length > 1+2)
            throw 'Too many arguments ('+arguments.length+' instead of '+(1+2)+')';
        return json_call_ajax(_url, "SubdataSelectionTester.hello_world",
            [name], 1, _callback, _errorCallback);
    };
    ...
    this.status = function (_callback, _errorCallback) {
        ...
        return json_call_ajax(_url, "SubdataSelectionTester.status",
